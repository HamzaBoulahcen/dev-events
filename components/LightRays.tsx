...
const LightRays: React.FC<LightRaysProps> = ({
  ...
  // Keep refs of props to avoid stale closures in WebGL resize/animation loops
  const raysOriginRef = useRef(raysOrigin);
  const followMouseRef = useRef(followMouse);

  // Update refs in an effect instead of during render
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      if (!active || !renderer || !uniformsRef.current || !meshRef.current) {
        return;
      }

      uniformsRef.current.iTime.value = t * 0.001;

      if (followMouseRef.current && uniformsRef.current.mouseInfluence.value > 0.0) {
    ...
    const initializeWebGL = async () => {
      ...
      // Small delay to ensure container dimensions are computed by browser
      ...
      // Clear any placeholder/old canvas
...
void main() {
  ...
  // Handle dynamic uniform updates without recreating the WebGL context
  useEffect(() => {
